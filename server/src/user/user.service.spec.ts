...
describe('User Service', () => {
	...
	describe("Create user", () => {
		it("should create first user (force admin)", async () => {
			adminUser = await userService.create({ name: 'admin', password: 'admin123456', admin: true });
		...
		it("should create first user (implicit admin)", async () => {
			adminUser = await userService.create({ name: 'admin1', password: 'admin123456', admin: false });
		...
		it("should create first lambda user ", async () => {
			user = await userService.create({ name: 'user', password: 'user12345', admin: false });
			expect(user.name).toBe('user');
			expect(user.admin).toBe(false);
			expect(user.enabled).toBe(false);
		});
		it("should fail, as user with same name already exists", async () => {
			const test = () => userService.create({ name: 'user', password: 'user12345', admin: false });
	...
	describe("Get user", () => {
		...
		it("Should find user by credentials", async () => {
			const fetchedUser = await userService.get({ byCredentials: { name: user.name, password: 'user12345' } });
			expect(fetchedUser).toStrictEqual(user);
		});
