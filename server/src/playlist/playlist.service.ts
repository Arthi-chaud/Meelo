...
> {
	...
	protected onCreationFailure(
		error: Error, input: PlaylistQueryParameters.CreateInput
	) {
	...
	protected formatCreateInputToWhereInput(input: PlaylistQueryParameters.CreateInput) {
		return {
			slug: new Slug(input.name)
		};
	}

	onNotFound(error: Error, where: PlaylistQueryParameters.WhereInput): Error | Promise<Error> {
		if (error instanceof Prisma.PrismaClientKnownRequestError
	...
	formatCreateInput(input: PlaylistQueryParameters.CreateInput): Prisma.PlaylistCreateInput {
		return {
			name: input.name,
	...
	formatSortingInput(sortingParameter: PlaylistQueryParameters.SortingParameter) {
	...
	formatUpdateInput(what: PlaylistQueryParameters.UpdateInput): Prisma.PlaylistUpdateInput {
		return {
			name: what.name,
	...
	formatDeleteInput(where: PlaylistQueryParameters.DeleteInput): Prisma.PlaylistWhereUniqueInput {
		return this.formatWhereInput(where);
	}

	protected formatDeleteInputToWhereInput(input: PlaylistQueryParameters.DeleteInput) {
		return input;
	}
	...
	async addSong(
		...
		const lastEntry = await this.prismaService.playlistEntry.findMany({
	...
	async flatten(where: PlaylistQueryParameters.WhereInput) {
		...
		return this.prismaService.$transaction(entries.map((entry, newIndex) =>
