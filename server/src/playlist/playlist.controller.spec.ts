...
describe('Playlist Controller', () => {
	...
	describe('Get Playlist', () => {
		it('Should get playlist (by ID)', async () => {
			return request(app.getHttpServer())
				...
				.expect((res) => {
					...
					expect(playlist).toStrictEqual(expectedPlaylistResponse(dummyRepository.playlist2));
				});
		});
		it('Should get playlist (by Slug)', async () => {
			return request(app.getHttpServer())
				...
				.expect((res) => {
					const playlist: Playlist = res.body;
					expect(playlist).toStrictEqual(expectedPlaylistResponse(dummyRepository.playlist3));
				});
		});
		...
		it('Should get playlist with entries', async () => {
			return request(app.getHttpServer())
				.get(`/playlists/${dummyRepository.playlist1.slug}?with=entries`)
				.expect(200)
				.expect((res) => {
					const playlist: Playlist = res.body;
		...
		it('Should Create Playlist', async () => {
			return request(app.getHttpServer())
				...
				.expect((res) => {
					...
					expect(newPlaylistResponse).toStrictEqual(expectedPlaylistResponse(newPlaylist));
				})
		});
	...
	describe('Add Song To Playlist', () => {
		it("Should Add Song to Playlist Entry", async () => {
			...
			expect(entries.at(3)!.index).toBe(dummyRepository.playlistEntry2.index + 1);
			expect(entries.at(3)!.songId).toBe(dummyRepository.songB1.id);
			await dummyRepository.playlistEntry.delete({ where: { id: entries.at(3)!.id } })
