...
@Injectable()
export default class WikipediaProvider extends IProvider<WikipediaSettings> implements OnModuleInit {
	constructor(
	...
	onModuleInit() {
		this._settings = this.settingsSettings.settingsValues.providers.wikipedia;
	}
	...
	async getResourceMetadataByWikidataId(resourceWikidataId: string) {
		try {
			const wikidataArticleName = await this.getWikipediaArticleName(resourceWikidataId);
			const description = await this.getWikipediaDescription(wikidataArticleName)
				.catch(() => null);

			return { value: wikidataArticleName, description };
		} catch {
			throw new ProviderActionFailedError(this.name, 'getResourceMetadataByWikidataId', 'Artist not found');
		}
	}
