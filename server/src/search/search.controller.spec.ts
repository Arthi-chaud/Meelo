...
describe("Search Controller", () => {
	...
	beforeAll(async () => {
		...
		(dummyRepository.artistA = await artistService.update(
			{ name: "Madonna" },
			{ id: dummyRepository.artistA.id },
		)),
			(dummyRepository.artistB = await artistService.update(
				{ name: "Goldfrapp" },
				{ id: dummyRepository.artistB.id },
			)),
			(dummyRepository.artistC = await artistService.update(
				{ name: "Doctor Rockit" },
				{ id: dummyRepository.artistC.id },
			)),
			(dummyRepository.albumA1 = await albumService.update(
				{ name: "American Life" },
				{ id: dummyRepository.albumA1.id },
			)),
			(dummyRepository.albumB1 = await albumService.update(
				{ name: "Tales of Us" },
				{ id: dummyRepository.albumB1.id },
			)),
			(dummyRepository.compilationAlbumA = await albumService.update(
				{ name: "Hotel Costes" },
				{ id: dummyRepository.compilationAlbumA.id },
			)),
			(dummyRepository.songA1 = await songService.update(
				{ name: "Hollywood" },
				{ id: dummyRepository.songA1.id },
			)),
			(dummyRepository.songA2 = await songService.update(
				{ name: "Intervention" },
				{ id: dummyRepository.songA2.id },
			)),
			(dummyRepository.songB1 = await songService.update(
				{ name: "Thea" },
				{ id: dummyRepository.songB1.id },
			)),
			(dummyRepository.songC1 = await songService.update(
				{ name: "Cafe de Flore" },
				{ id: dummyRepository.songC1.id },
			));
		dummyRepository.releaseA1_1 = await releaseService.update(
			{ name: "American Life (Edited)" },
			{ id: dummyRepository.releaseA1_1.id },
		);
		dummyRepository.releaseA1_2 = await releaseService.update(
			{ name: "American Life (Special Edition)" },
			{ id: dummyRepository.releaseA1_2.id },
		);
		dummyRepository.releaseB1_1 = await releaseService.update(
			{ name: "Tales of Us" },
			{ id: dummyRepository.releaseB1_1.id },
		);
		dummyRepository.compilationReleaseA1 = await releaseService.update(
			{ name: "Hotel Costes (Deluxe)" },
			{ id: dummyRepository.compilationReleaseA1.id },
		);
		dummyRepository.genreC = await genreService.update(
			{ name: "Electronic" },
