...
const buildSearchUrl = (type: string | undefined) => {
	return `/search/${type ? `?t=${type}` : ""}`;
};

const tabs = ["all", "artist", "album", "song", "video"] as const;

const SearchPage: Page<GetPropsTypesFrom<typeof prepareSSR>> = () => {
	...
	return (
		...
		>
			<TabPage
				tabs={tabs}
				urlFromTab={(tab) => buildSearchUrl(tab)}
				translateTab={(tab) =>
					tab === "all" ? "search.all" : `models.${tab}_plural`
				}
				header={
					<>
						...
						case "album":
							return (
								<InfiniteAlbumView
									...
									onItemClick={(item) =>
										item &&
										saveSearch.mutate({ albumId: item.id })
									}
						...
						case "song":
							return (
								<InfiniteSongView
									...
									onItemClick={(item) =>
										item &&
										saveSearch.mutate({ songId: item.id })
									}
									query={({ libraries, types }) =>
										getSongs(
											{
						...
						case "video":
							return (
								<InfiniteVideoView
									onItemClick={(item) =>
										item &&
										saveSearch.mutate({ videoId: item.id })
									}
									disableSort
									subtitle="artist"
									query={({ libraries, types }) =>
